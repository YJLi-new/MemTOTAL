...
BASE_SEED="${1:-61109}"
RUN_ROOT="${2:-/root/autodl-tmp/runs/verify/planv8-v8-5-bridge-revisit}"
RESULT_ROOT="${3:-/root/autodl-tmp/results/generated/planv8-v8-5-bridge-revisit}"
PRIMARY_MODEL_DIR="${4:-${PRIMARY_MODEL_DIR_DEFAULT}}"
V84_RUN_ROOT="${5:-/root/autodl-tmp/runs/verify/planv8-v8-4-external-writer}"
V84_SUMMARY_PATH="${6:-results/generated/review/planv8-v8-4-external-writer/v8-4-summary.json}"
SELECTED_PROMPTS_PATH="${7:-results/generated/review/planv8-v8-4-external-writer/selected-prompt-modes.json}"
...
if next_step != "open_v8_5_bridge":
    raise SystemExit(
        f"V8-4 did not authorize V8-5; recommended_next_step={next_step!r}"
    )
PY
...
summary = json.loads(Path(sys.argv[1]).read_text())
base_arm_id = str(summary.get("base_for_v8_5_arm_id") or summary.get("best_arm_id") or "").strip()
if not base_arm_id:
    raise SystemExit("V8-4 summary did not provide base_for_v8_5_arm_id or best_arm_id.")
print(base_arm_id)
PY
)"
...
cp "${V84_SUMMARY_PATH}" "${RESULT_ROOT}/v8-4-summary.reference.json"

materialize_config() {
  local task_name="$1"
  local arm_id="$2"
  local output_config="$3"
  local base_config="${V84_RUN_ROOT}/materialized-configs/${task_name}-${BASE_ARM_ID}.json"
  local base_checkpoint="${V84_RUN_ROOT}/${BASE_ARM_ID}-${task_name}/checkpoint.pt"
  python scripts/planv8_v8_5_config.py \
...
python scripts/update_planv8_v8_5_summary.py \
  --result_root "${RESULT_ROOT}" \
  --v84_summary "${V84_SUMMARY_PATH}" \
  --output_json "${RESULT_ROOT}/v8-5-summary.json" \
  --output_report "${RESULT_ROOT}/v8-5-summary.md"

bash scripts/publish_review_artifacts.sh

echo "PLANv8 V8-5 bridge revisit complete."
