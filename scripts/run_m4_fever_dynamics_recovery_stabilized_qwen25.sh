...
BASE_SEED="${1:-12601}"
RUN_ROOT="${2:-runs/verify/m4-fever-dynamics-recovery-stabilized-qwen25}"
RESULT_ROOT="${3:-results/generated/m4-fever-dynamics-recovery-stabilized-qwen25}"
...
for SUITE in "${SUITES[@]}"; do
  ...
  python scripts/run_m4_selected_shared_injection_suite.py \
    --config configs/exp/m4_fever_qwen25_phase2_val_stabilized_common.yaml \
...
./scripts/run_analysis.sh \
  --config configs/exp/m4_fever_qwen25_dynamics_recovery_stabilized.yaml \
  --seed "$((BASE_SEED + 900))" \
...
if [[ "${SELECTION_PASSED}" == "1" ]]; then
  python scripts/run_m4_fixed64_gate_from_selection.py \
    --config configs/exp/m4_fever_qwen25_fixed64_gate_stabilized_common.yaml \
...
rsync -a --exclude='*.pt' --exclude='*.ckpt' \
  "${RUN_ROOT}/" "runs/review/m4-fever-dynamics-recovery-stabilized-qwen25/"
rsync -a --exclude='*.pt' --exclude='*.ckpt' \
  "${RESULT_ROOT}/" "results/generated/review/m4-fever-dynamics-recovery-stabilized-qwen25/"

./scripts/publish_review_artifacts.sh

echo "m4 FEVER stabilized dynamics recovery complete (selection_passed=${SELECTION_PASSED})"
