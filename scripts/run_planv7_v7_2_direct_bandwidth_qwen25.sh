...
BASE_SEED="${1:-61109}"
RUN_ROOT="${2:-/root/autodl-tmp/runs/verify/planv7-v7-2-direct-bandwidth-qwen25}"
RESULT_ROOT="${3:-/root/autodl-tmp/results/generated/planv7-v7-2-direct-bandwidth-qwen25}"
...
config["runtime"]["pilot_active_depth_family"] = "D1"
...
if arm_id == "control":
    ...
    config["runtime"]["pilot_deep_prefix_layers"] = [12, 13, 14, 15]
...
config["experiment"]["name"] = f"{os.environ['PLANV7_EXPERIMENT_PREFIX']}_v7_2_{task_name}_{arm_id}"
...
for arm_id in d_w1_shared d_w2_shared d_w2_perlayer; do
  for task_name in gsm8k triviaqa fever; do
    materialize_config \
      "${task_name}" \
      "${arm_id}" \
...
python scripts/update_planv7_v7_2_direct_bandwidth_summary.py \
  --result_root "${RESULT_ROOT}" \
  --output_json "${RESULT_ROOT}/v7-2-primary-summary.json" \
  --output_report "${RESULT_ROOT}/v7-2-primary-summary.md"

mapfile -t promoted_arms < <(python - "${RESULT_ROOT}/v7-2-primary-summary.json" <<'PY'
import json
import sys

payload = json.loads(open(sys.argv[1]).read())
for arm_id in payload.get("promoted_arms", [])[:2]:
    print(str(arm_id))
PY
)
...
ensure_suite_complete \
  ...
  "$((BASE_SEED + seed_offset))" \
  "${RUN_ROOT}/fever-control" \
  "pilot-C0:C0:base_only:real:0"
copy_run_artifacts \
  "${RESULT_ROOT}/control/fever" \
  "${RUN_ROOT}/fever-control" \
  "pilot-C0"
seed_offset=$((seed_offset + 20))

for arm_id in "${promoted_arms[@]}"; do
  ensure_suite_complete \
    "${CONFIG_ROOT}/fever-${arm_id}.json" \
    "$((BASE_SEED + seed_offset))" \
    "${RUN_ROOT}/fever-${arm_id}" \
    "pilot-I-real:${arm_id^^}:injected:real:0"
  copy_run_artifacts \
    "${RESULT_ROOT}/${arm_id}/fever" \
    "${RUN_ROOT}/fever-${arm_id}" \
    "pilot-I-real"
  seed_offset=$((seed_offset + 20))
done
...
python scripts/update_planv7_v7_2_direct_bandwidth_summary.py \
  --result_root "${RESULT_ROOT}" \
  --output_json "${RESULT_ROOT}/v7-2-summary.json" \
  --output_report "${RESULT_ROOT}/v7-2-summary.md"

bash scripts/publish_review_artifacts.sh

echo "PLANv7 V7-2 direct bandwidth ladder complete."
