...
BASE_SEED="${1:-18291}"
RUN_ROOT="${2:-/root/autodl-tmp/runs/verify/tl-slot-basis-rescue-fever-qwen25}"
RESULT_ROOT="${3:-/root/autodl-tmp/results/generated/tl-slot-basis-rescue-fever-qwen25}"
...
BRIDGE_RESCUE_SUMMARY="${9:-results/generated/review/tl-bridge-rescue-fever-qwen25/tl-h4-k8-rescue/run-summary.json}"
BASELINE_TL_READER_CSV="${10:-results/generated/review/tl-poc-fever-qwen25/tl-h4-k8/dynamics-recovery/reader_query_diagnostics.csv}"
BRIDGE_RESCUE_READER_CSV="${11:-results/generated/review/tl-bridge-rescue-fever-qwen25/tl-h4-k8-rescue/dynamics-recovery/reader_query_diagnostics.csv}"
BASELINE_TL_TRAIN_EVENTS="${12:-runs/review/tl-poc-fever-qwen25/tl-h4-k8/phase2-selected/pilot-I-real/train_events.json}"
BRIDGE_RESCUE_TRAIN_EVENTS="${13:-runs/review/tl-bridge-rescue-fever-qwen25/tl-h4-k8-rescue/phase2-selected/pilot-I-real/train_events.json}"
...
BRIDGE_RESCUE_SUMMARY="$(resolve_path "${BRIDGE_RESCUE_SUMMARY}")"
BASELINE_TL_READER_CSV="$(resolve_path "${BASELINE_TL_READER_CSV}")"
BRIDGE_RESCUE_READER_CSV="$(resolve_path "${BRIDGE_RESCUE_READER_CSV}")"
BASELINE_TL_TRAIN_EVENTS="$(resolve_path "${BASELINE_TL_TRAIN_EVENTS}")"
BRIDGE_RESCUE_TRAIN_EVENTS="$(resolve_path "${BRIDGE_RESCUE_TRAIN_EVENTS}")"
...
output_path.write_text(json.dumps({"tl-h4-k8-slot-basis-rescue": str(warm_start)}, indent=2, sort_keys=True) + "\n")
PY

python - "configs/exp/tl_slot_basis_rescue_fever_qwen25_h4_k8.yaml" "${MATERIALIZED_CONFIG_ROOT}/tl_slot_basis_rescue_fever_qwen25_h4_k8.json" "${WARM_START}" <<'PY'
import json
import sys
from pathlib import Path
...
RUN_NAME="tl-h4-k8-slot-basis-rescue"
...
if [[ ! -f "${RUN_DIR}/phase2-selected/suite_metrics.json" ]]; then
  python scripts/run_m4_selected_shared_injection_suite.py \
    --config "${MATERIALIZED_CONFIG_ROOT}/tl_slot_basis_rescue_fever_qwen25_h4_k8.json" \
...
if [[ ! -f "${RESULT_DIR}/dynamics-recovery/selection.json" ]]; then
  ./scripts/run_analysis.sh \
    --config configs/exp/tl_slot_basis_rescue_fever_qwen25_dynamics_recovery_h4_k8.yaml \
    --seed "$((BASE_SEED + 900))" \
...
if [[ "${SELECTION_PASSED}" == "1" ]]; then
  if [[ ! -f "${RUN_DIR}/screen248-test-canonical/pilot-I-zero-gate/metrics.json" ]]; then
    python scripts/run_m4_gate_from_selection.py \
      --config "configs/exp/tl_slot_basis_rescue_fever_qwen25_screen248_test_gate_h4_k8.yaml" \
  ...
  if [[ "${CANONICAL_PASSED}" == "1" ]]; then
    if [[ ! -f "${RUN_DIR}/screen248-test-heldout-a/pilot-I-zero-gate/metrics.json" ]]; then
      python scripts/run_m4_gate_from_selection.py \
        --config "configs/exp/tl_slot_basis_rescue_fever_qwen25_screen248_test_heldout_a_gate_h4_k8.yaml" \
        --selection_json "${SELECTION_JSON}" \
        --resume "${RESUME_STAGE_B_ROOT}" \
    ...
    if [[ ! -f "${RUN_DIR}/screen248-test-heldout-b/pilot-I-zero-gate/metrics.json" ]]; then
      python scripts/run_m4_gate_from_selection.py \
        --config "configs/exp/tl_slot_basis_rescue_fever_qwen25_screen248_test_heldout_b_gate_h4_k8.yaml" \
        --selection_json "${SELECTION_JSON}" \
        --resume "${RESUME_STAGE_B_ROOT}" \
...
if [[ "${PROMOTE_TO_FIXED64}" == "1" ]]; then
  if [[ ! -f "${RUN_DIR}/fixed64-legacy/pilot-I-zero-gate/metrics.json" ]]; then
    python scripts/run_m4_gate_from_selection.py \
      --config "configs/exp/tl_slot_basis_rescue_fever_qwen25_fixed64_gate_h4_k8.yaml" \
      --selection_json "${SELECTION_JSON}" \
      --resume "${RESUME_STAGE_B_ROOT}" \
...
python scripts/update_tl_slot_basis_summary.py \
  --sl8_summary_json "${BASELINE_SL8_SUMMARY}" \
  --tl_h4_k8_summary_json "${BASELINE_TL_SUMMARY}" \
  --tl_bridge_rescue_summary_json "${BRIDGE_RESCUE_SUMMARY}" \
  --tl_slot_basis_summary_json "${RESULT_DIR}/run-summary.json" \
  --tl_h4_k8_reader_query_csv "${BASELINE_TL_READER_CSV}" \
  --tl_bridge_rescue_reader_query_csv "${BRIDGE_RESCUE_READER_CSV}" \
  --tl_slot_basis_reader_query_csv "${RESULT_DIR}/dynamics-recovery/reader_query_diagnostics.csv" \
  --tl_h4_k8_train_events_json "${BASELINE_TL_TRAIN_EVENTS}" \
  --tl_bridge_rescue_train_events_json "${BRIDGE_RESCUE_TRAIN_EVENTS}" \
  --tl_slot_basis_train_events_json "${RUN_DIR}/phase2-selected/pilot-I-real/train_events.json" \
  --output_json "${RESULT_ROOT}/slot-basis-summary.json" \
  --output_report "${RESULT_ROOT}/slot-basis-summary.md"

./scripts/publish_review_artifacts.sh

echo "TL slot-basis rescue complete"
