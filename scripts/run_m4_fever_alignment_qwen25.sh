...
BASE_SEED="${1:-12991}"
RUN_ROOT="${2:-/root/autodl-tmp/runs/verify/m4-fever-shared-injection-alignment-qwen25}"
RESULT_ROOT="${3:-/root/autodl-tmp/results/generated/m4-fever-shared-injection-alignment-qwen25}"
...
run_suite() {
  ...
  if [[ "${selection_passed}" == "1" ]]; then
    ...
    ./scripts/run_analysis.sh \
      --config configs/exp/m4_fever_qwen25_compare_m47.yaml \
...
run_suite \
  "canonical" \
  "configs/exp/m4_fever_qwen25_phase2_val_alignment_canonical_common.yaml" \
  "configs/exp/m4_fever_qwen25_dynamics_recovery_m47_structured.yaml" \
  "configs/exp/m4_fever_qwen25_screen248_test_gate_m47_structured_common.yaml" \
  "configs/exp/m4_fever_qwen25_screen248_test_heldout_a_gate_m47_structured_common.yaml" \
  "configs/exp/m4_fever_qwen25_screen248_test_heldout_b_gate_m47_structured_common.yaml" \
  "configs/exp/m4_fever_qwen25_fixed64_gate_m47_structured_legacy_common.yaml" \
  "$((BASE_SEED + 100))"

run_suite \
  "freeze-writer" \
  "configs/exp/m4_fever_qwen25_phase2_val_alignment_freeze_writer_common.yaml" \
  "configs/exp/m4_fever_qwen25_dynamics_recovery_m47_structured.yaml" \
  "configs/exp/m4_fever_qwen25_screen248_test_gate_m47_structured_common.yaml" \
  "configs/exp/m4_fever_qwen25_screen248_test_heldout_a_gate_m47_structured_common.yaml" \
  "configs/exp/m4_fever_qwen25_screen248_test_heldout_b_gate_m47_structured_common.yaml" \
...
run_suite \
  "pooled-block" \
  "configs/exp/m4_fever_qwen25_phase2_val_alignment_pooled_block_common.yaml" \
  "configs/exp/m4_fever_qwen25_dynamics_recovery_m47_pooled.yaml" \
  "configs/exp/m4_fever_qwen25_screen248_test_gate_m47_pooled_common.yaml" \
  "configs/exp/m4_fever_qwen25_screen248_test_heldout_a_gate_m47_pooled_common.yaml" \
  "configs/exp/m4_fever_qwen25_screen248_test_heldout_b_gate_m47_pooled_common.yaml" \
  "configs/exp/m4_fever_qwen25_fixed64_gate_m47_pooled_legacy_common.yaml" \
  "$((BASE_SEED + 700))"

python scripts/update_m4_alignment_summary.py \
...
rsync -a --exclude='*.pt' --exclude='*.ckpt' \
  "${RUN_ROOT}/" "runs/review/m4-fever-shared-injection-alignment-qwen25/"
rsync -a --exclude='*.pt' --exclude='*.ckpt' \
  "${RESULT_ROOT}/" "results/generated/review/m4-fever-shared-injection-alignment-qwen25/"

./scripts/publish_review_artifacts.sh

echo "m4 FEVER shared injection alignment complete"
