...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="PLANv8 V8-7 split-pool retrieval baseline eval.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    initialize_run_artifacts(
        ...
        config=config,
        seed=args.seed,
        argv=sys.argv if argv is None else ["planv8_v8_7_retrieval_eval", *argv],
    )
    ...
    metrics = {
        "mode": "eval_baseline_rag_split_pool",
        ...
        "support_pool_size": len(support_dataset),
        "mean_support_retrieval_score": (
            support_score_total / max(1, support_score_count) if support_score_count > 0 else 0.0
        ),
    }
    if str(config["task"].get("benchmark_id", "")).strip().lower() == "fever":
        metrics["macro_f1"] = mean_score
