...
BASE_SEED="${1:-61109}"
RUN_ROOT="${2:-/root/autodl-tmp/runs/verify/planv8-v8-4-external-writer}"
RESULT_ROOT="${3:-/root/autodl-tmp/results/generated/planv8-v8-4-external-writer}"
PRIMARY_MODEL_DIR="${4:-${PRIMARY_MODEL_DIR_DEFAULT}}"
V83_RUN_ROOT="${5:-/root/autodl-tmp/runs/verify/planv8-v8-3-reader-opd}"
V83_SUMMARY_PATH="${6:-results/generated/review/planv8-v8-3-reader-opd/v8-3-summary.json}"
SELECTED_PROMPTS_PATH="${7:-results/generated/review/planv8-v8-3-reader-opd/selected-prompt-modes.json}"
...
if next_step != "open_v8_4_external_writer":
    raise SystemExit(
        f"V8-3 did not authorize V8-4; recommended_next_step={next_step!r}"
    )
PY
...
summary = json.loads(Path(sys.argv[1]).read_text())
base_arm_id = str(summary.get("base_for_v8_4_arm_id") or summary.get("best_arm_id") or "").strip()
if not base_arm_id:
    raise SystemExit("V8-3 summary did not provide base_for_v8_4_arm_id or best_arm_id.")
print(base_arm_id)
PY
)"
...
materialize_config() {
  ...
  local base_config="${V83_RUN_ROOT}/materialized-configs/${task_name}-${BASE_ARM_ID}.json"
  local base_checkpoint="${V83_RUN_ROOT}/${BASE_ARM_ID}-${task_name}/checkpoint.pt"
  python scripts/planv8_v8_4_config.py \
...
python scripts/update_planv8_v8_4_summary.py \
  --result_root "${RESULT_ROOT}" \
  --v83_summary "${V83_SUMMARY_PATH}" \
  --output_json "${RESULT_ROOT}/v8-4-summary.json" \
  --output_report "${RESULT_ROOT}/v8-4-summary.md"

bash scripts/publish_review_artifacts.sh

echo "PLANv8 V8-4 external writer complete."
