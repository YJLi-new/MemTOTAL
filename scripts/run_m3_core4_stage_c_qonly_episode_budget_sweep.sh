...
SEED_BASE="${1:-15201}"
RUNS_ROOT="${2:-/root/autodl-tmp/memtotal-stage-c-qonly-episode-budget-sweep}"
REPORT_DIR="${3:-results/generated/m3-core4-stage-c-qonly-episode-budget-sweep}"
...
write_override() {
  ...
  local repeats="$5"
  {
    ...
    printf '  target_episode_repeats: %s\n' "${repeats}"
  } > "${path}"
}
...
for repeats in 1 3 5; do
  write_override \
    "${TMP_CONFIG_DIR}/qwen25_ep${repeats}.yaml" \
    "${ROOT_DIR}/configs/exp/m3_stage_c_core4_qwen25_smoke.yaml" \
    "m3_stage_c_core4_qwen25_qonly_episode_budget_${repeats}" \
    "m3-core4-stage-c-qonly-episodes-${repeats}-qwen25" \
    "${repeats}"
  write_override \
    "${TMP_CONFIG_DIR}/qwen3_ep${repeats}.yaml" \
    "${ROOT_DIR}/configs/exp/m3_stage_c_core4_qwen3_smoke.yaml" \
    "m3_stage_c_core4_qwen3_qonly_episode_budget_${repeats}" \
    "m3-core4-stage-c-qonly-episodes-${repeats}-qwen3" \
    "${repeats}"
done
...
for ((index=0; index<SEED_COUNT; index++)); do
  ...
  for repeats in 1 3 5; do
    run_stage_c \
      "${TMP_CONFIG_DIR}/qwen25_ep${repeats}.yaml" \
      "${qwen25_seed}" \
      "${RUNS_ROOT}/qwen25-ep${repeats}-seed-${qwen25_seed}" \
      "${QWEN25_STAGEB}"
    run_stage_c \
      "${TMP_CONFIG_DIR}/qwen3_ep${repeats}.yaml" \
      "${qwen3_seed}" \
      "${RUNS_ROOT}/qwen3-ep${repeats}-seed-${qwen3_seed}" \
      "${QWEN3_STAGEB}"
  done
done
