...
PRIMARY_ARM_PREFIX="${PLANV8_PRIMARY_ARM_PREFIX:-q3}"
...
PRIMARY_MODEL_DIR_DEFAULT="${PLANV8_PRIMARY_MODEL_DIR:-/root/autodl-tmp/models/${PRIMARY_BACKBONE_NAME}}"
PRIMARY_GSM8K_NONTHINK_PROMPT_VARIANT="${PLANV8_PRIMARY_GSM8K_NONTHINK_PROMPT_VARIANT:-q3_gsm8k_nonthink}"
PRIMARY_GSM8K_THINK_PROMPT_VARIANT="${PLANV8_PRIMARY_GSM8K_THINK_PROMPT_VARIANT:-q3_gsm8k_think_boxed}"
PRIMARY_TRIVIA_NONTHINK_PROMPT_VARIANT="${PLANV8_PRIMARY_TRIVIA_NONTHINK_PROMPT_VARIANT:-q3_trivia_nonthink}"
PRIMARY_TRIVIA_THINK_PROMPT_VARIANT="${PLANV8_PRIMARY_TRIVIA_THINK_PROMPT_VARIANT:-q3_trivia_think}"

BASE_SEED="${1:-61109}"
RUN_ROOT="${2:-/root/autodl-tmp/runs/verify/planv8-v8-0-${PRIMARY_BACKBONE_KEY}-baselines-oracles}"
RESULT_ROOT="${3:-/root/autodl-tmp/results/generated/planv8-v8-0-${PRIMARY_BACKBONE_KEY}-baselines-oracles}"
QWEN25_MODEL_DIR="${4:-/root/autodl-tmp/models/Qwen2.5-1.5B-Instruct}"
PRIMARY_MODEL_DIR="${5:-${PRIMARY_MODEL_DIR_DEFAULT}}"
...
materialize_config() {
  ...
  local train_path="$6"
  local eval_path="$7"
  python - "${task_name}" "${arm_id}" "${prompt_variant}" "${output_config}" "${support_path}" "${train_path}" "${eval_path}" "${QWEN25_MODEL_DIR}" "${PRIMARY_MODEL_DIR}" "${PRIMARY_BACKBONE_NAME}" <<'PY'
import json
import sys
from pathlib import Path
...
primary_model_dir = str(Path(sys.argv[9]).resolve())
primary_backbone_name = sys.argv[10]
...
config["backbone"]["name"] = "Qwen2.5-1.5B-Instruct" if use_qwen25 else primary_backbone_name
config["backbone"]["model_id"] = qwen25_model_dir if use_qwen25 else primary_model_dir
...
if arm_id.startswith(("b0_", "b1_", "b2_", "b3_", "b4_")):
    runtime["shared_injection_arm"] = "base_only"
...
BASELINE_ARMS=(
  "gsm8k:b0_${PRIMARY_ARM_PREFIX}_gsm8k_nonthink:${PRIMARY_GSM8K_NONTHINK_PROMPT_VARIANT}"
  "gsm8k:b1_${PRIMARY_ARM_PREFIX}_gsm8k_think_boxed:${PRIMARY_GSM8K_THINK_PROMPT_VARIANT}"
  "triviaqa:b2_${PRIMARY_ARM_PREFIX}_trivia_nonthink:${PRIMARY_TRIVIA_NONTHINK_PROMPT_VARIANT}"
  "triviaqa:b3_${PRIMARY_ARM_PREFIX}_trivia_think:${PRIMARY_TRIVIA_THINK_PROMPT_VARIANT}"
  "fever:b4_${PRIMARY_ARM_PREFIX}_fever_nonthink:answer_slot_labels"
)
...
GSM8K_ARM_A="b0_${PRIMARY_ARM_PREFIX}_gsm8k_nonthink"
GSM8K_ARM_B="b1_${PRIMARY_ARM_PREFIX}_gsm8k_think_boxed"
TRIVIAQA_ARM_A="b2_${PRIMARY_ARM_PREFIX}_trivia_nonthink"
TRIVIAQA_ARM_B="b3_${PRIMARY_ARM_PREFIX}_trivia_think"
FEVER_ARM="b4_${PRIMARY_ARM_PREFIX}_fever_nonthink"

GSM8K_SELECTION="$(select_prompt_variant "gsm8k" "${GSM8K_ARM_A}" "${GSM8K_ARM_B}")"
TRIVIAQA_SELECTION="$(select_prompt_variant "triviaqa" "${TRIVIAQA_ARM_A}" "${TRIVIAQA_ARM_B}")"
python - <<'PY' "${RESULT_ROOT}" "${GSM8K_SELECTION}" "${TRIVIAQA_SELECTION}" "${FEVER_ARM}"
import json
import sys
from pathlib import Path
...
ORACLE_ARMS=(
  ...
  "gsm8k:o1_${PRIMARY_ARM_PREFIX}_prefix_oracle_mid4:${GSM8K_PROMPT}"
  "triviaqa:o1_${PRIMARY_ARM_PREFIX}_prefix_oracle_mid4:${TRIVIAQA_PROMPT}"
  "gsm8k:o2_${PRIMARY_ARM_PREFIX}_seq_oracle16:${GSM8K_PROMPT}"
  "triviaqa:o2_${PRIMARY_ARM_PREFIX}_seq_oracle16:${TRIVIAQA_PROMPT}"
  "gsm8k:o3_${PRIMARY_ARM_PREFIX}_seq_oracle32:${GSM8K_PROMPT}"
  "triviaqa:o3_${PRIMARY_ARM_PREFIX}_seq_oracle32:${TRIVIAQA_PROMPT}"
  "gsm8k:o4_${PRIMARY_ARM_PREFIX}_xattn_oracle_smoke:${GSM8K_PROMPT}"
  "triviaqa:o4_${PRIMARY_ARM_PREFIX}_xattn_oracle_smoke:${TRIVIAQA_PROMPT}"
)
...
python scripts/update_planv8_v8_0_summary.py \
  ...
  --primary-backbone-key "${PRIMARY_BACKBONE_KEY}" \
  --primary-backbone-label "${PRIMARY_BACKBONE_NAME}" \
  --primary-arm-prefix "${PRIMARY_ARM_PREFIX}"
