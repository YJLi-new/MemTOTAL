...
SEED_BASE="${1:-17201}"
RUNS_ROOT="${2:-/root/autodl-tmp/memtotal-stage-c-qonly-target-split-sweep}"
REPORT_DIR="${3:-results/generated/m3-core4-stage-c-qonly-target-split-sweep}"
...
write_override() {
  ...
  local split_policy="$5"
  {
    ...
    printf '  target_support_weighting: uniform\n'
    printf '  target_split_policy: %s\n' "${split_policy}"
  } > "${path}"
}
...
for split_policy in random proxy_topk_support proxy_bottomk_support; do
  write_override \
    "${TMP_CONFIG_DIR}/qwen25_${split_policy}.yaml" \
    "${ROOT_DIR}/configs/exp/m3_stage_c_core4_qwen25_smoke.yaml" \
    "m3_stage_c_core4_qwen25_qonly_target_split_${split_policy}" \
    "m3-core4-stage-c-qonly-target-split-${split_policy}-qwen25" \
    "${split_policy}"
  write_override \
    "${TMP_CONFIG_DIR}/qwen3_${split_policy}.yaml" \
    "${ROOT_DIR}/configs/exp/m3_stage_c_core4_qwen3_smoke.yaml" \
    "m3_stage_c_core4_qwen3_qonly_target_split_${split_policy}" \
    "m3-core4-stage-c-qonly-target-split-${split_policy}-qwen3" \
    "${split_policy}"
done
...
for ((index=0; index<SEED_COUNT; index++)); do
  ...
  for split_policy in random proxy_topk_support proxy_bottomk_support; do
    run_stage_c \
      "${TMP_CONFIG_DIR}/qwen25_${split_policy}.yaml" \
      "${qwen25_seed}" \
      "${RUNS_ROOT}/qwen25-${split_policy}-seed-${qwen25_seed}" \
      "${QWEN25_STAGEB}"
    run_stage_c \
      "${TMP_CONFIG_DIR}/qwen3_${split_policy}.yaml" \
      "${qwen3_seed}" \
      "${RUNS_ROOT}/qwen3-${split_policy}-seed-${qwen3_seed}" \
      "${QWEN3_STAGEB}"
  done
done
