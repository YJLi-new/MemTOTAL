...
SEED_BASE="${1:-13201}"
RUNS_ROOT="${2:-/root/autodl-tmp/memtotal-stage-c-qonly-policy-sweep}"
REPORT_DIR="${3:-results/generated/m3-core4-stage-c-qonly-policy-sweep}"
...
write_override() {
  ...
  local policy="$5"
  {
    ...
    printf '\nruntime:\n'
    printf '  target_episode_policy: %s\n' "${policy}"
  } > "${path}"
}
...
for policy in independent aggregate_support; do
  write_override \
    "${TMP_CONFIG_DIR}/qwen25_${policy}.yaml" \
    "${ROOT_DIR}/configs/exp/m3_stage_c_core4_qwen25_smoke.yaml" \
    "m3_stage_c_core4_qwen25_qonly_policy_${policy}" \
    "m3-core4-stage-c-qonly-policy-${policy}-qwen25" \
    "${policy}"
  write_override \
    "${TMP_CONFIG_DIR}/qwen3_${policy}.yaml" \
    "${ROOT_DIR}/configs/exp/m3_stage_c_core4_qwen3_smoke.yaml" \
    "m3_stage_c_core4_qwen3_qonly_policy_${policy}" \
    "m3-core4-stage-c-qonly-policy-${policy}-qwen3" \
    "${policy}"
done
...
for ((index=0; index<SEED_COUNT; index++)); do
  ...
  for policy in independent aggregate_support; do
    run_stage_c \
      "${TMP_CONFIG_DIR}/qwen25_${policy}.yaml" \
      "${qwen25_seed}" \
      "${RUNS_ROOT}/qwen25-${policy}-seed-${qwen25_seed}" \
      "${QWEN25_STAGEB}"
    run_stage_c \
      "${TMP_CONFIG_DIR}/qwen3_${policy}.yaml" \
      "${qwen3_seed}" \
      "${RUNS_ROOT}/qwen3-${policy}-seed-${qwen3_seed}" \
      "${QWEN3_STAGEB}"
  done
done
