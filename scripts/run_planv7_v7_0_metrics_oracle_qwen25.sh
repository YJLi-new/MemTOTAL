...
BASE_SEED="${1:-61109}"
RUN_ROOT="${2:-/root/autodl-tmp/runs/verify/planv7-v7-0-metrics-oracle-qwen25}"
RESULT_ROOT="${3:-/root/autodl-tmp/results/generated/planv7-v7-0-metrics-oracle-qwen25}"
...
materialize_config() {
  ...
  local train_path="$5"
  local eval_path="$6"
  python - "${task_name}" "${variant}" "${output_config}" "${support_path}" "${train_path}" "${eval_path}" "${TRAIN_STEPS}" "${MODEL_DIR}" <<'PY'
import json
import os
import sys
...
if variant in {"c_mid", "o_ctx_mid", "o_sup_mid"}:
    depth_layers = [12, 13, 14, 15]
    receiver_layers = [12, 13, 14, 15]
elif variant == "c_add":
    depth_layers = [0, 1, 2, 3, 4, 8, 14]
    receiver_layers = [0, 1, 2, 3, 4]

config["runtime"]["pilot_deep_prefix_layers"] = depth_layers
config["method"]["receiver_lora"] = dict(receiver_enabled)
config["method"]["receiver_lora"]["target_layers"] = list(receiver_layers)
...
if variant == "control":
    ...
    config["method"]["receiver_lora"] = receiver_disabled
    if variant.startswith("o_ctx"):
        config["runtime"]["pilot_prefix_source_mode"] = "oracle_context_echo"
    else:
        config["runtime"]["pilot_prefix_source_mode"] = "oracle_support_echo"
else:
    raise ValueError(f"Unsupported V7-0 variant={variant}")

config["experiment"]["name"] = f"{os.environ['PLANV7_EXPERIMENT_PREFIX']}_v7_0_{task_name}_{variant}"
...
for variant in c_add c_early c_mid; do
  for task_name in gsm8k triviaqa fever; do
    materialize_config \
      "${task_name}" \
      "${variant}" \
...
for variant in o_ctx_early o_ctx_mid o_sup_early o_sup_mid; do
...
for variant in c_add c_early c_mid; do
  arm_alias="$(printf '%s' "${variant}" | tr '[:lower:]' '[:upper:]')"
  pilot_subdir="pilot-${arm_alias}"
  for task_name in gsm8k triviaqa fever; do
    ensure_suite_complete \
      "${CONFIG_ROOT}/${task_name}-${variant}.json" \
      "$((BASE_SEED + seed_offset))" \
      "${RUN_ROOT}/${task_name}-${variant}" \
      "${pilot_subdir}:${arm_alias}:injected:real:0"
    copy_run_artifacts \
      "${RESULT_ROOT}/${variant}/${task_name}" \
      "${RUN_ROOT}/${task_name}-${variant}" \
      "${pilot_subdir}"
    seed_offset=$((seed_offset + 100))
  done
...
python scripts/update_planv7_v7_0_metrics_oracle_summary.py \
  --result_root "${RESULT_ROOT}" \
  --output_json "${RESULT_ROOT}/v7-0-summary.json" \
  --output_report "${RESULT_ROOT}/v7-0-summary.md"
