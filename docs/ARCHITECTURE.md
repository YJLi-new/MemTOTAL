...
当前 M3 smoke 已经把 toy 路径与 benchmark-native `core4` 路径都接进统一 artifact contract、resume 链路与 summary。当前 benchmark-native `core4` 还只是 smoke 级协议验证，不代表正式 few-shot 结果；但 canonical 配置现已从早期 `smoke4/2x2` 升级为 `smoke8/3x3`，并进一步通过 Stage B probe suite 收口到 backbone-specific episode budget：qwen25 当前 canonical `meta_episodes=16`，`runs/verify/m3-core4-qwen25/stage-b/metrics.json` 记录 `mean_adaptation_gain=6.527453660964966e-05`；qwen3 当前 canonical 仍为 `meta_episodes=6`，`runs/verify/m3-core4-qwen3/stage-b/metrics.json` 记录 `mean_adaptation_gain=0.0007965167363484701`。正式 probe suite `results/generated/m3-core4-stage-b-probe-suite-v2/metrics.json` 当前也显示两档 backbone 的最佳变体都回到各自 canonical。Stage C 这条线则已经从“q-only 基本不承载更新”推进到“q-only 在 objective 上真实可动，但 task score 仍偏钝”：reader 现通过 `query_residual_scale` 把 gated query residual 直接注入 `readouts`，默认配置 `configs/method/memory_bootstrap.yaml` 已锁到 `1.0`。新的 sensitivity audit `results/generated/m3-stage-c-sensitivity-audit/` 记录 qwen25 `query_to_memory_score_delta_ratio=0.7457097764552213`、qwen3 `query_to_memory_score_delta_ratio=1.2928086655448818`，说明 query path 对最终 candidate score 的函数影响已经抬到与 memory path 同量级。对应的 fresh Stage C probe `results/generated/m3-core4-stage-c-probe-suite-v2/metrics.json` 现在显示：两档 backbone 的 `q_only` 都已变成 `adaptation_effective=True`，qwen25 的 q-only gradient audit 为 `queries_grad_norm=0.002236595833389479`、`query_to_writer_grad_ratio=0.07231639531004393`，qwen3 则为 `queries_grad_norm=0.0004174626431004455`、`query_to_writer_grad_ratio=0.057166275150394304`；同时 q-only 的 `best_adapt_query_loss` 已与 `w_only / w_plus_q` 基本打平。新的 q-only budget probe `results/generated/m3-core4-stage-c-qonly-budget-probe-suite-v2/metrics.json` 也说明预算已不再是主阻塞：两档 backbone 的所有预算变体都已经 `adaptation_effective=True`，qwen25 最优 objective 出现在 `lr=5.0, steps=10`，而 qwen3 的 canonical `lr=0.2, steps=3` 已是最优。当前剩下的真正 blocker 已经从“q-only 参数化失效”收缩成“smoke 级 `task_score` 仍对 objective 改善不够敏感，且不同 target seed 上绝对分数波动较大”。
