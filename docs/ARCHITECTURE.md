...
- Stage C：
  ...
  - 新增 `src/memtotal/analysis/m3_stage_c_probe.py` 与 `configs/exp/m3_stage_c_probe_summary.yaml`；probe suite 会把 `Stage C` 的 `q_only / w_only / w_plus_q` 放到同一份 summary 中，并显式检查同一 backbone 下三条曲线是否共用同一个 seed

当前 M3 smoke 已经把 toy 路径与 benchmark-native `core4` 路径都接进统一 artifact contract、resume 链路与 summary。当前 benchmark-native `core4` 还只是 smoke 级协议验证，不代表正式 few-shot 结果；但 canonical 配置现已从早期 `smoke4/2x2` 升级为 `smoke8/3x3`，并进一步通过 Stage B probe suite 收口到 backbone-specific episode budget：qwen25 当前 canonical `meta_episodes=16`，`runs/verify/m3-core4-qwen25/stage-b/metrics.json` 记录 `mean_adaptation_gain=6.527453660964966e-05`；qwen3 当前 canonical 仍为 `meta_episodes=6`，`runs/verify/m3-core4-qwen3/stage-b/metrics.json` 记录 `mean_adaptation_gain=0.0007965167363484701`。正式 probe suite `results/generated/m3-core4-stage-b-probe-suite-v2/metrics.json` 当前也显示两档 backbone 的最佳变体都回到各自 canonical。与此同时，Stage C 现在已经能把“target task_score 不动”进一步诊断成“更新几乎为零”：qwen25 canonical `Stage C` 当前记录 `adaptation_effective=False`、`mean_support_grad_norm=2.0708178345536896e-08`、`max_support_update_max_abs=9.313225746154785e-10`。进一步的 gradient audit `results/generated/m3-stage-c-gradient-audit-qwen25/metrics.json` 还显示 `queries_grad_norm=2.0715830091178723e-08`，而同一 loss 下 `fuser_grad_norm=0.11522613661274937`、`writer_grad_norm=0.012883167814417517`。最新的 Stage C probe suite `results/generated/m3-core4-stage-c-probe-suite/metrics.json` 又把这个问题收紧了一步：在同一 backbone 共用同一个 target seed 的前提下，两档 backbone 的 `q_only` 都仍然 `adaptation_effective=False`；qwen25 的 `q_only / w_only / w_plus_q` 三条线在 `task_score` 和 `best_adapt_query_loss` 上完全打平，而 qwen3 只有 writer-inclusive 变体能把 `best_adapt_query_loss` 从 `1.5973001718521118` 压到 `1.596140742301941`。接着的 q-only budget probe `results/generated/m3-core4-stage-c-qonly-budget-probe-suite/metrics.json` 又进一步说明这不是简单调大预算就能解决的问题：即便把 `adapt_learning_rate` 提到 `5.0`、`adapt_steps` 提到 `10`，两档 backbone 的 q-only 仍然全部 `adaptation_effective=False`，qwen25 的最好 objective 只到 `1.6321028470993042`，qwen3 也仍停在 `1.6052712202072144`。最后，新的 sensitivity audit `results/generated/m3-stage-c-sensitivity-audit/` 又给出了更直接的函数层证据：在相同 `+0.1` counterfactual 扰动下，qwen25 的 `query_to_memory_score_delta_ratio=1.1986101721896082e-05`，qwen3 的对应比值只有 `4.128581513429619e-06`。也就是说，query 路径对最终 candidate score 的影响比 memory path 小了大约 5 到 6 个数量级。这说明当前下一个明确 blocker 已从 Stage B 稳定性转向 Stage C 的 q-only parameterization，以及 smoke task score 对 objective 改善的敏感度不足。
