...
当前 M3 smoke 已经把 toy 路径与 benchmark-native `core4` 路径都接进统一 artifact contract、resume 链路与 summary。当前 benchmark-native `core4` 还只是 smoke 级协议验证，不代表正式 few-shot 结果；但 canonical 配置现已从早期 `smoke4/2x2` 升级为 `smoke8/3x3`，并进一步通过 Stage B probe suite 收口到 backbone-specific episode budget：qwen25 当前 canonical `meta_episodes=16`，`runs/verify/m3-core4-qwen25/stage-b/metrics.json` 记录 `mean_adaptation_gain=6.527453660964966e-05`；qwen3 当前 canonical 仍为 `meta_episodes=6`，`runs/verify/m3-core4-qwen3/stage-b/metrics.json` 记录 `mean_adaptation_gain=0.0007965167363484701`。正式 probe suite `results/generated/m3-core4-stage-b-probe-suite-v2/metrics.json` 当前也显示两档 backbone 的最佳变体都回到各自 canonical。Stage C 这条线则已经分成四步收口：先通过 `query_residual_scale` 修复 q-only 参数化，让 query path 真正进入有效控制回路；再通过 `task_proxy_score` 把 coarse `task_score` 打平时的 target-side变化显式暴露出来；随后把 canonical target 评测升级为 `target_eval_repeats=3` 的多 query-set 官方聚合；最新再补了一条 `q_only` target-seed sweep，把单 seed 的正向结果放回分布里看。当前 sensitivity audit `results/generated/m3-stage-c-sensitivity-audit/` 记录 qwen25 `query_to_memory_score_delta_ratio=0.7457097764552213`、qwen3 `query_to_memory_score_delta_ratio=1.2928086655448818`，说明 query path 对最终 candidate score 的函数影响已经抬到与 memory path 同量级。fresh Stage C probe `results/generated/m3-core4-stage-c-probe-suite-v4/metrics.json` 进一步显示：两档 backbone 的 `q_only` 都已 `adaptation_effective=True`，并且官方 `task_score` 不再完全冻结。qwen25 在同一 target seed 上三条线都从 `0.6666666666666666 -> 0.8888888888888888`，qwen3 的三条线都从 `0.5555555555555555 -> 0.7777777777777778`；这说明当前 official metric 已经能随 query adaptation 真正移动。对应的 q-only budget probe `results/generated/m3-core4-stage-c-qonly-budget-probe-suite-v4/metrics.json` 也显示：qwen25 的 official `accuracy` 现在能在同一 target seed 上从 `0.3333333333333333 -> 0.5555555555555555`，最佳预算仍是 `lr=5.0, steps=10`；但 qwen3 在这组 seed 上仍表现为 `0.4444444444444444 -> 0.2222222222222222`。而 fresh `results/generated/m3-core4-stage-c-qonly-seed-sweep/metrics.json` 则进一步说明：把 canonical `q_only` 放到 5 个 target seeds 上后，qwen25 当前 `positive_gain_rate=0.2`、`mean_task_gain=-0.2`，qwen3 当前 `positive_gain_rate=0.4`、`mean_task_gain=-0.13333333333333333`。因此，当前剩下的真正 blocker 已经进一步收缩成“single-seed official gain 已存在，但 target-side 稳定性仍不足，均值尚未转正”，下一步应优先升级 target support/query 的多 episode 聚合，而不是继续怀疑 q-only 路径本身。
