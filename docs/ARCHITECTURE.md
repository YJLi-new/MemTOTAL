...
- Stage C：
  ...
  - `runtime.target_split_policy` 现支持 `random / proxy_topk_support / proxy_bottomk_support`；当前 canonical `core4` Stage C 配置已切到 `proxy_bottomk_support`，即优先把 target-side proxy 较差的样本放进 support，再把更平滑的样本留给 query/eval
