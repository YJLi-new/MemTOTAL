...
- Stage C：
  ...
  - `runtime.target_split_policy` 现支持 `random / proxy_topk_support / proxy_bottomk_support`；当前 runner 默认仍用 `proxy_bottomk_support`，但在修正后的 fixed-holdout eval 下尚未重新完成公平 sweep，因此它现在只算暂定默认值
