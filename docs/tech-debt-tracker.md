...
- benchmark-native `Stage C` 当前暴露出一个更具体的新 blocker：qwen25 canonical `q_only` 适配虽然会执行 `support_updates`，但更新量极小，`runs/verify/m3-core4-qwen25/stage-c/metrics.json` 当前记录 `adaptation_effective=False`、`mean_support_grad_norm=2.0708178345536896e-08`、`max_support_update_max_abs=9.313225746154785e-10`。这说明问题不只是“task_score 没涨”，而是“q-only 在当前 benchmark-native 路径上几乎没有有效更新”。
- `results/generated/m3-stage-c-gradient-audit-qwen25/metrics.json` 进一步表明这不是单纯的学习率问题：当前 qwen25 canonical support loss 下 `queries_grad_norm=2.0715830091178723e-08`，而 `fuser_grad_norm=0.11522613661274937`、`writer_grad_norm=0.012883167814417517`。也就是说，当前 benchmark-native 路径上的梯度主要不落在 queries 上；若后续仍坚持 `q_only` 作为默认正式口径，需要解释这种梯度分配，或补更适合 q-only 的 parameterization。
- `results/generated/m3-core4-stage-c-probe-suite/metrics.json` 说明当前 blocker 还有第二层：即便把 Stage C 适配对象消融放到同 seed、同 target episode 上，qwen25 的 `q_only / w_only / w_plus_q` 仍然在 `task_score` 和 `best_adapt_query_loss` 上完全打平；qwen3 只有 writer-inclusive 变体能略降 objective loss，但 `task_score` 仍不动。这说明后续不仅要解决 q-only 梯度过小，还要让 benchmark-native smoke 的 target metric 对 objective 改善更敏感。
- `results/generated/m3-core4-stage-c-qonly-budget-probe-suite/metrics.json` 进一步表明这也不是简单的 Stage C budget 不足：当前把 q-only 的 `adapt_learning_rate` 扫到 `5.0`、`adapt_steps` 扫到 `10` 后，两档 backbone 仍然全部 `adaptation_effective=False`。这说明后续如果还想坚持默认 `q_only`，更该优先改 query parameterization / readout path，而不是继续盲扫更大的少步数预算。
