...
- `grid.reuse_existing_runs` 现已落地并在 protocol-smoke suite 上验证，可避免仅因导入点/汇总变化而重复执行整套 grid；但目前它依赖 `metrics.json / predictions.jsonl / checkpoint.pt` 的存在性判断，还没有更严格的 config hash 校验。
