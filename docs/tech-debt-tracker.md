...
- 因此，当前剩下的 tech debt 已经更具体：不是缺可观测性，而是 official target metric 仍然过粗。后续要么增大 target episode/query 样本数，要么在不违背主表口径的前提下补更稳的 official aggregation，才能把已经恢复的 proxy-level 改善转成 paper-facing `task_score` 提升。
