...
- 因此，当前下一步的 tech debt 优先级已经很明确：不是继续做 q-only 是否有效的 existence proof，而是把 `Stage C` 升级到 target support/query 多 episode 聚合或更稳的 target-split 口径，减少当前 `worst_task_gain` 仍可掉到 `-0.5555555555555556`（qwen25）和 `-0.4444444444444444`（qwen3）的 seed 崩塌。
