...
- 因此，当前最值得继续拆的已不再是“要不要加 source negatives”，而是“如何在保持公平 eval 的前提下把 source-augmented support negatives 再做强”，优先看更丰富的 negative sampling 与 harder negatives，而不是继续调 split policy；`support_bank_size` 现在已经从“怀疑点”变成“已证实有帮助但单独不够”的次级杠杆。
