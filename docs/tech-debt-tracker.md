...
- `M5` 的 `prompting` baseline family 现已接入 `Vanilla / CoT`，但当前仍只是 qwen25 的 zero-shot smoke 验证；后续还需要补到两档 backbone、真实 benchmark smoke、few-shot/step 预算对齐，以及主表需要的 Prompt Tuning / LoRA / MetaPrompting 家族。
