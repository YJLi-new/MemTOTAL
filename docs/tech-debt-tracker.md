...
- `results/generated/m3-stage-c-sensitivity-audit/` 现在给出了更直接的函数层证据：在相同 `+0.1` counterfactual 扰动下，qwen25 的 `query_to_memory_score_delta_ratio=1.1986101721896082e-05`，qwen3 只有 `4.128581513429619e-06`。这说明即使不看梯度，query 路径对最终 score 的函数影响也远小于 memory path；后续若继续修 `q_only`，应优先看 query 注入到 readouts/summary 的有效性，而不是只看优化器超参。
