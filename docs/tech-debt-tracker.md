...
- `M5` 的 `prompting` baseline family 现已接入 `Vanilla / CoT`，并补到了两档固定 backbone；但当前仍只是 `GSM8K + Story Cloze` 的 zero-shot smoke 验证，后续还需要补真实 benchmark smoke、few-shot/step 预算对齐，以及主表需要的 Prompt Tuning / LoRA / MetaPrompting 家族。
