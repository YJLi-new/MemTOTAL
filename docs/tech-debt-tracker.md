...
- 因此，当前最值得继续拆的已不再是“要不要加 source negatives”，也不再是“该不该切 hard sampler”，而是“为什么最强 proxy sampler 仍然推不动 official gain”。下一步优先看 `hard_by_current_model` 下的 step saturation / support budget，而不是继续调 split policy、只改 bank size，或回退到更弱 sampler；`support_bank_size` 现在已经从“怀疑点”变成“已证实有帮助但单独不够”的次级杠杆。
