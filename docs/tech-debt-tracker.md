...
- 因此，当前最值得继续拆的已不再是“要不要加 source negatives”，也不再是“该不该切 hard sampler”，而是“为什么 qwen3 的负 margin seeds 会被 inner-loop step 推向更坏方向”。下一步优先看负 margin seeds 的条件化 step 方向分析，而不是继续调 split policy、只改 bank size，或回退到更弱 sampler；`support_bank_size` 现在已经从“怀疑点”变成“已证实有帮助但单独不够”的次级杠杆。
