...
- `Stage C` 现在还会额外产出 `episode_trace.json`。fresh diagnose run `runs/diagnose/m3-qwen3-negative-seed-{40301,40304}/episode_trace.json` 已经证明 qwen3 的两个负 seed 是明显双峰，而不是单纯噪声：`40301` 会随着 step 单调变差，`40304` 会随着 step 单调变好。因此，当前最值得继续拆的已进一步收缩成“哪些 support 组成会把 qwen3 的负 margin seeds 推向更坏方向”。下一步优先做 support 组成的条件化分析或相应采样策略，而不是继续调 split policy、只改 bank size，或回退到更弱 sampler；`support_bank_size` 现在已经从“怀疑点”变成“已证实有帮助但单独不够”的次级杠杆。
