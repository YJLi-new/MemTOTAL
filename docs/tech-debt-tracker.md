...
- `M5` 的 `adapter` baseline family 现已接入最小 `Prompt Tuning / LoRA` 闭环，并补到了两档固定 backbone；但当前仍只支持 candidate-selection 任务的 smoke 训练，并且只在 `story_cloze` 上验证；后续还需要补到更多任务和更正式的 few-shot/step 网格。
