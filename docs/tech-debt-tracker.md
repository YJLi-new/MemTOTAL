...
- `target_support_negative_sampler={deterministic_id, hard_by_continuation}` 的 fresh 对照 `results/generated/m3-core4-stage-c-qonly-support-negative-sampler-sweep-v1/metrics.json` 现进一步说明：hard-by-continuation 还不是稳定杠杆。qwen25 的 `mean_proxy_gain` 从 `1.106e-5` 提升到 `1.950e-5`，但 qwen3 的 `mean_proxy_gain` 从 `3.389e-5` 回落到 `3.102e-5`，两档 backbone 的 official `mean_task_gain` 都仍为 `0.0`。
- 因此，当前最值得继续拆的已不再是“要不要加 source negatives”，而是“如何在保持公平 eval 的前提下把 source-augmented support negatives 再做强”，优先看更细的 source-negative 难度构造，而不是继续调 split policy、只改 bank size，或直接切 `hard_by_continuation`；`support_bank_size` 现在已经从“怀疑点”变成“已证实有帮助但单独不够”的次级杠杆。
