...
- benchmark-native `core4_transfer_smoke` 现已打通真实 benchmark 子集上的 `Stage A/B/C` 协议，并已做一轮 retrieval follow-up：query/val 侧候选池排除 support continuations，inner-loop 只在 support pool 内做 retrieval。当前 canonical `Stage B` 的 `mean_adaptation_gain` 已从 `-0.4` 级别压到 `-3.577272097269694e-05`（qwen25）与 `-5.6743621826171875e-05`（qwen3），但仍未形成稳定正收益。
