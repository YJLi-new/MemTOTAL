...
- 2026-03-09: Implemented the V4 multitask bridge surface: `BackboneWrapper.generate(...)` now accepts shallow/deep prefix inputs, `src/memtotal/training/m4_shared_injection.py` now supports task-native benchmark prompts plus exact-match / QA-F1 generative scoring inside the shared-injection pilot, `scripts/run_m4_selected_shared_injection_suite.py` now accepts custom arm lists without FEVER phase-0 inputs, and the new V4 task templates / runner / summary live under `configs/exp/tl_multitask_qwen25_*_template.yaml`, `scripts/run_tl_bridge_multitask_qwen25.sh`, and `scripts/update_tl_multitask_summary.py`.
- 2026-03-09: Completed the real Step-6 Qwen2.5 V4 multitask run at `/root/autodl-tmp/runs/verify/tl-bridge-multitask-qwen25`, then synced the review bundle into `runs/review/tl-bridge-multitask-qwen25` and `results/generated/review/tl-bridge-multitask-qwen25`.
- 2026-03-09: The real V4 multitask results were negative. NarrativeQA control reached `task_score≈0.2857` (`qa_f1`) on the 1-example eval slice, but the late3 micro-LoRA family regressed to `0.0`; GSM8K stayed flat at `0.0` exact match for both control and bridge. Across both tasks, the bridge kept the same `memory_long_top1_top2_ratio≈70.7` and `reader_readout_effective_rank≈1.22` geometry seen on FEVER even though early Reader/Fuser/receiver-LoRA grad medians were much larger on the non-FEVER smokes.
- 2026-03-09: Recorded the real Step-6 decision as `comparison_conclusion=failure`, `primary_interpretation=no_multitask_signal`, `any_nonfever_positive=false`, `move_to_v5=false`, and `stop_after_v4=true`; per `PLANv3.md`, Qwen3 confirmation and later Stage B/C refresh remain locked.
...
- The real V4 summary is terminal for `PLANv3` progression: do not open V5 or V6, because the bridge showed no positive non-FEVER signal and NarrativeQA regressed versus the frozen control.
...
- The V4 smokes sharpened that conclusion rather than rescuing it: outside FEVER the bridge can carry large optimization signals, but those signals still do not translate into better answers while the Writer common-mode domination and readout collinearity remain essentially unchanged.
