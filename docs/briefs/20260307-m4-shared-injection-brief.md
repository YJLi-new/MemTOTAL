...
`shared injection` 已经不是“完全没信号”，但截至 `M4.5` 仍没有在预注册 `screen248-val` 规则下稳定选出 checkpoint。当前最真实的 blocker 已经从 `shallow prefix norm blow-up` 演化成：

> main-chain consumption 已成立，deep prompt 也已成立；但当前系统还会把可训练信号迅速推到 cap 边界，并塌成强标签偏置，而不是稳定的 real-memory content effect。
...
> 但无论 shallow 还是 deep，当前训练动力学都还不能把这种内容效应稳定固化成通过预注册 validation 的能力信号。
...
- 保持 `triad6` 为唯一 primary support variant
- 保持 `screen-train / screen-val / screen248-test / fixed64` 的职责隔离
- 继续增强 observability，重点盯：
  - cap saturation
  - real-vs-shuffle 的层内分离
  - label-bias collapse
- 下一轮优先考虑：
  - 更强 bottleneck / regularization
  - 更严格限制 projector 自由度
  - 若 deep prompt 仍不过 gate，则进入 writer objective 重写
