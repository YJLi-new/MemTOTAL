...
说明：benchmark-native `core4` smoke 现在已经打通真实 benchmark 子集上的 `Stage A/B/C` artifact contract、多 source meta-split 与真实 `task_score` 曲线；最新 follow-up 进一步修掉了 Stage C 里 `shot`-耦合 target episode / query pool / support pool 的协议泄漏，并把 `target_split_policy={random, proxy_topk_support, proxy_bottomk_support}` 在公平 fixed-holdout 口径下重新 sweep 了一遍。现在还额外有十二套正式 probe/curve harness：Stage B probe 用于比较 backbone-specific 预算，Stage C probe 用于在同 seed 下比较 `q_only / w_only / w_plus_q`，Stage C q-only budget probe 用于验证不同 `lr/steps` 下的 q-only 行为，Stage C sensitivity audit 用于直接比较 query path 和 memory path 的函数影响量级，Stage C q-only target-split sweep 用于直接比较 target support/query 抽样结构，Stage C q-only support-bank sweep 用于直接比较 support bank 规模，Stage C q-only support-negative-pool sweep 用于直接比较 support negatives 来源，Stage C q-only support-negative-sampler sweep 用于直接比较 negatives 排序策略，Stage C curve suite 用于直接产出更接近正式协议的 `shot_curve / step_curve`，Stage C step saturation audit 用于把 `zero->step0` 与 `step0->final` 两段收益拆开，Stage C margin audit 用于直接检查离 rank flip 还差多少，Stage C negative-seed curve audit 用于只看错误 seeds 上的 shot/step 作用。同时，Stage C 现在已有并行的 `task_proxy_score` 观测层：当 official `task_score` 因为样本太少或指标太粗而打平时，仍能继续观测 `gold_choice_probability` 这种更平滑的 target-side 变化。当前最新证据已经说明：benchmark-native Stage C 的主 blocker 已不再是 `q_only` 参数化无效，也不再是 `target_split_policy` 选择；`target_support_bank_size=auto` 已证明“有帮助但单独不够”，`target_support_negative_pool=source_plus_support_bank` 已成为第一条真正更强的公平杠杆，而最新 `target_support_negative_sampler={deterministic_id, hard_by_continuation, hard_by_current_model}` 对照进一步显示：`hard_by_current_model` 现在已经成为当前最强的 proxy 杠杆。再往前一步，新的 canonical `curve suite / step saturation audit`、`margin audit` 与 `negative-seed curve audit` 共同说明：inner-loop steps 确实会稳定累加 proxy gain，也会在一部分 seeds 上继续推高 mean margin；但这些增益并没有体现在 `shot` 的纯作用上，而且在真正错误的 seeds 上，qwen25 与 qwen3 的 step 方向甚至相反。下一步应直接围绕“为什么 qwen3 的负 margin seeds 会被 step 推向更坏方向”做条件化分析或采样策略，而不是继续怀疑 inner-loop 根本没学到东西。
