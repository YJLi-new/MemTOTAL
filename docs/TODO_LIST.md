...
- benchmark-native `core4` smoke：
  ...
  - 当前剩余 blocker 已从“q-only 几乎不承载更新”收缩成“benchmark-native smoke 的 `task_score` 对 objective 改善仍不敏感，而且不同 target seed 上绝对分数会波动”；例如 probe-suite-v2 上两档 backbone 都停在 `0.6666666666666666 -> 0.6666666666666666`，而 q-only budget probe v2 的 qwen25 又落在 `0.3333333333333333 -> 0.3333333333333333`
...
说明：benchmark-native `core4` smoke 现在已经打通真实 benchmark 子集上的 `Stage A/B/C` artifact contract、多 source meta-split 与真实 `task_score` 曲线；最新 canonical follow-up 已进一步把 episode 结构提升到 `smoke8/3x3`，并在“query eval 排除 support、support inner-loop 只看 support pool”的 episode-aware retrieval 协议下，把两档 backbone 的 Stage B `mean_adaptation_gain` 都翻成了正值。现在还额外有四套正式 probe harness：Stage B probe 用于比较 backbone-specific 预算，Stage C probe 用于在同 seed 下比较 `q_only / w_only / w_plus_q`，Stage C q-only budget probe 用于验证不同 `lr/steps` 下的 q-only 行为，Stage C sensitivity audit 用于直接比较 query path 和 memory path 的函数影响量级。当前最新证据已经说明：benchmark-native Stage C 的主 blocker 已不再是 `q_only` 参数化无效，而是 smoke 级 `task_score` 对 objective 改善仍然不够敏感，后续需要更强的 target metric 或更稳定的 target episode 设计来承接已经恢复的 q-only 更新信号。
