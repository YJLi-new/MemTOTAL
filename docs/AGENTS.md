...
> 更深的知识请写入并维护在：`TODO_LIST.md`、`MAIN_IDEA.md`、`EXPERIMENTS_INFO.md`、`docs/exec-plans/` 与 `docs/`。

---

## 0) 一句话 TL;DR（永远先做这件事）

**任何任务开始之前：先打开并严格按 `TODO_LIST.md` 执行。**

- `TODO_LIST.md`：唯一入口 / 任务编排 / DoD / 产物要求 / 里程碑顺序
...
> 3. 若文档冲突：**方法口径以 `MAIN_IDEA.md` 为准；实验口径以 `EXPERIMENTS_INFO.md` 为准；执行顺序与验收以 `TODO_LIST.md` 为准。**

---

## 1) 文档分工（先分工，再做事）

### `TODO_LIST.md` —— 单一权威 runbook / backlog
所有任务都从这里开始。它负责：
- 里程碑顺序（M0–M6）
- P0/P1/P2 优先级
...
- `TODO_LIST.md`：任务分解、DoD、优先级、runbook
...
1. **Ask / Understand**  
   先在 `TODO_LIST.md` 里定位对应里程碑、条目与 DoD。  
   不允许脱离 TODO “凭感觉”做事。
...
- 维护 `docs/` 中的 golden principles / quality tracker / tech-debt tracker（若 TODO 要求）
