...
def run_story_cloze_real_pilot_split(
    *,
    ...
    write_json(
        ...
        {
            "analysis_mode": "story_cloze_real_pilot_split",
            "source_dataset_path": str(source_dataset_path),
            "support_dataset_path": str(support_path.resolve()),
            "eval_dataset_path": str(eval_path.resolve()),
            "support_examples": len(support_rows),
...
def _select_bucket_rows(
    ...
    *,
    ...
    if bucket == "base_correct_control":
        near = sorted(rows, key=lambda row: (float(row["screening_base_margin"]), str(row["id"])))[: min(10, quota)]
...
def run_story_cloze_real_fixed_set_builder(
    *,
    ...
    candidate_runs: list[tuple[dict[str, Any], Path]] = []
    for metrics_path in sorted(root.rglob("metrics.json")):
        metrics = json.loads(metrics_path.read_text())
        if metrics.get("training_stage") != "stage_c_real_pilot":
            continue
        candidate_runs.append((metrics, metrics_path.parent))
    base_runs = [
        (metrics, run_dir)
        for metrics, run_dir in candidate_runs
        if str(metrics.get("decision_mode")) == "base_only"
    ]
    shared_runs = [
        (metrics, run_dir)
        for metrics, run_dir in candidate_runs
        if str(metrics.get("decision_mode")) == "shared_summary_late_fusion"
        and str(metrics.get("choice_objective")) == "continuation_retrieval"
        and str(metrics.get("memory_control_mode")) == "real"
    ]
    def _pick_screening_run(candidates: list[tuple[dict[str, Any], Path]]) -> tuple[dict[str, Any], Path] | None:
        screening_candidates = [
            (metrics, run_dir)
            for metrics, run_dir in candidates
            if str(metrics.get("pilot_split")) == "screen_eval248"
            or str(metrics.get("eval_dataset_path", "")).endswith("screen-eval248.jsonl")
        ]
        if len(screening_candidates) == 1:
            return screening_candidates[0]
        if len(screening_candidates) > 1:
            screening_candidates.sort(key=lambda item: str(item[1]))
            return screening_candidates[-1]
        if len(candidates) == 1:
            return candidates[0]
        return None

    picked_base = _pick_screening_run(base_runs)
    picked_shared = _pick_screening_run(shared_runs)
...
def run_stage_c_real_pilot_compare(
    *,
    ...
    def _pick_fixed100_run(candidates: list[tuple[dict[str, Any], Path]]) -> tuple[dict[str, Any], Path] | None:
        fixed_candidates = [
            (metrics, run_dir)
            for metrics, run_dir in candidates
            if str(metrics.get("pilot_split")) == "fixed100"
            or str(metrics.get("eval_dataset_path", "")).endswith("fixed100.jsonl")
        ]
        if len(fixed_candidates) == 1:
            return fixed_candidates[0]
        if len(fixed_candidates) > 1:
            fixed_candidates.sort(key=lambda item: str(item[1]))
            return fixed_candidates[-1]
        return None

    runs: dict[str, tuple[dict[str, Any], Path]] = {}
    missing = []
    for alias, key in required_keys.items():
        picked = _pick_fixed100_run(runs_by_key.get(key, []))
        if picked is None:
    ...
    for example_id in sorted(set(selected_rows_by_alias["A"]) & set(selected_rows_by_alias["C"])):
            ...
            flip_rows.append(
                {
                    ...
                    "story": str(candidate_row.get("story", "")),
