...
# Dataset configs
DATASET_NAME="${DATASET_NAME:-gsm8k}"  # options: gsm8k, gpqa, kodcode, triviaqa, cosmosqa, rocstories, story_cloze
...
if [ -z "${LOAD_MODEL_PATH:-}" ]; then
  if [ -f "${LOCAL_QWEN}/model.safetensors" ]; then
    LOAD_MODEL_PATH="${LOCAL_QWEN}/model.safetensors"
  else
    LOAD_MODEL_PATH="$(pwd)/data_chache/models/Qwen2.5-1.5B-Instruct/model.safetensors"
  fi
fi
