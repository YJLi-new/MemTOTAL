...
# Dataset configs
DATASET_NAME="${DATASET_NAME:-gsm8k}"  # keep gsm8k for trigger training to compare on the same evaluation
...
# Load the sequentially fine-tuned weaver (gsm8k -> rocstories)
DEFAULT_WEAVER_PATH="$(pwd)/results/train/rocstories/ssd/pn=1_pl=16_in=5_il=8_20251212-152134/weaver"
LOAD_WEAVER_PATH="${LOAD_WEAVER_PATH:-${DEFAULT_WEAVER_PATH}}"

if [ ! -e "${LOAD_WEAVER_PATH}" ]; then
  echo "Weaver checkpoint not found at LOAD_WEAVER_PATH=${LOAD_WEAVER_PATH}" >&2
  exit 1
fi
