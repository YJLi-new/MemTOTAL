...
class MemGenModel(PreTrainedModel, MemGenLoraSwitchMixin, MemGenGenerationMixin):
    ...
    @classmethod
    def _load_causal_lm(cls, model_name: str, attn_implementation: str | None):
        ...
        try:
            ...
            if attn_implementation == "flash_attention_2":
                logging.warning(
                    "flash_attention_2 is unavailable for %s; falling back to sdpa.",
                    model_name,
                )
    ...
    def from_config(cls, config_dict: dict):
        ...
        base_model = cls._load_causal_lm(
            model_name,
        ...
        if weaver_model_name != model_name:
            weaver_model = cls._load_causal_lm(
                weaver_model_name,
                attn_implementation=attn_implementation,
            )
        else:
        ...
        if trigger_model_name != model_name:
            trigger_model = cls._load_causal_lm(
                trigger_model_name,
                attn_implementation=attn_implementation,
            )
        else:
