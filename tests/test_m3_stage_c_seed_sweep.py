...
class M3StageCSeedSweepTest(unittest.TestCase):
            ...
            json.dumps(
                {
                    ...
                    "target_support_negative_sampler": "deterministic_id",
                    "support_updates": 3,
    ...
    def test_analysis_mode_writes_seed_sweep_summary(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertEqual(
                metrics["by_backbone"]["Qwen2.5-1.5B-Instruct"]["target_support_weightings"],
                ["uniform"],
            )
            self.assertEqual(
            ...
            self.assertEqual(
                metrics["by_backbone"]["Qwen2.5-1.5B-Instruct"]["target_support_bank_sizes"],
                ["auto"],
            )
            self.assertEqual(
            ...
            self.assertEqual(
                metrics["by_backbone"]["Qwen2.5-1.5B-Instruct"]["target_support_negative_samplers"],
                ["deterministic_id"],
            )
            self.assertEqual(
